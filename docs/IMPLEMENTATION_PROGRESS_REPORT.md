...
- `market.audit.query` - 审计记录查询
- `web3.audit.anchor` - 链上锚定
...
- Gate-SEC-01: ⚠️ `web3.index.list` 返回了敏感的 `endpoint` 字段

---

## 🚧 进行中的工作（P0阻断项）

### P0-1: Gate-SEC-01（敏感信息零泄露）❌ 未达标

**问题描述**：
多处API返回了敏感信息：

- `web3.index.list` 返回 Provider endpoint
- 错误消息包含真实文件路径
- 日志可能泄露 accessToken
