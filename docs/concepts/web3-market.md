...
- **你（用户）只做少量决策**：发布什么、价格/规则、要不要租用别人、以及（可选）选择支付网络（当前默认 EVM；TON 主路径已实现）。
...
2. **选择支付网络并租用**：当前默认走 EVM（Base/Sepolia）；TON 已实现端到端结算（见双栈策略）；AI 管家签发租约并代管一次性 token（不回显）。
...
关于 TON+EVM 双栈并行（用户选链支付、后台统一对账口径）的实现详情，见：[双栈策略总规划](/web3/WEB3_DUAL_STACK_STRATEGY)（含当前边界说明，如 TON provider 事件轮询 TODO）。
