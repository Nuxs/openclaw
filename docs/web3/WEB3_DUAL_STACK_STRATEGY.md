...
- 不把 `extensions/blockchain-adapter` 误读为"已完成全部双栈能力"：它已作为**执行层**被 `market-core` 的 EVM escrow（以及 `agent-wallet`）复用；现阶段 EVM 与 TON 双栈主路径可用，但 TON provider 事件轮询链路仍有 TODO 边界。
...
- **支付资产与汇率口径不足**：双链可付带来 FX 与对账追溯问题，需要统一报价来源、有效期与取整规则（`floor/ceil/nearest`）。
