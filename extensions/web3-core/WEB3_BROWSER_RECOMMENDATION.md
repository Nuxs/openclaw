...
- **桌面端优先**：我建议先做一个最小浏览器扩展原型：
  - 能捕获 `window.ethereum.request` 的方法名与来源站点
  - 能把事件发到本机 Gateway
  - Gateway 落一条审计记录（复用你们现有 Web3 Core 的审计模块）

如果你告诉我你更关注 **桌面端（Brave/Chrome）** 还是 **App 内（macOS/iOS/Android WebView）**，我可以把“最小原型”直接落到仓库里（包含扩展目录结构、与 Gateway 的事件协议、以及一条端到端 demo）。
