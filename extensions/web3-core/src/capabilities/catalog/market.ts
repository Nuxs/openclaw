...
export function marketCapabilities(config: Web3PluginConfig): CapabilityDescriptor[] {
  ...
  return [
    ...
    {
      name: "web3.market.order.list",
      summary: "List market orders with status/actor filters.",
      kind: "gateway",
      group: "market",
      availability: availability(resourcesEnabled, "resources disabled"),
      paramsSchema: {
        ...
        properties: {
          orderId: { type: "string" },
          offerId: { type: "string" },
          buyerId: { type: "string", pattern: "^0x[a-fA-F0-9]{40}$" },
          sellerId: { type: "string", pattern: "^0x[a-fA-F0-9]{40}$" },
          status: {
            type: "string",
            enum: [
              "active",
    ...
    },
    {
      ...
      summary: "Build a redacted quote summary for a published market resource.",
      kind: "gateway",
      group: "market",
      availability: availability(resourcesEnabled, "resources disabled"),
      paramsSchema: {
      ...
      returns:
        "Redacted quote payload with price, estimated total, proof requirements, and service metadata summary.",
      risk: { level: "low" },
    },
    {
      name: "web3.market.offer.compare",
      summary: "Compare candidate published resources for the same buyer intent.",
      kind: "gateway",
      group: "market",
      availability: availability(resourcesEnabled, "resources disabled"),
      paramsSchema: {
        type: "object",
        properties: {
          kind: { type: "string", enum: ["model", "search", "storage", "service"] },
          tag: { type: "string" },
          query: { type: "string" },
          quantity: { type: "number", minimum: 1, maximum: 1000 },
      ...
      returns: "Ranked candidate list with quote previews and comparable scores.",
      risk: { level: "low" },
    },
    {
      name: "web3.market.settlement.query",
      summary: "Query settlement records and aggregate earnings metrics.",
      kind: "gateway",
      group: "market",
      availability: availability(resourcesEnabled, "resources disabled"),
      paramsSchema: {
        ...
        properties: {
          ...
          status: {
            type: "string",
            enum: ["settlement_locked", "settlement_released", "settlement_refunded"],
          },
      ...
      returns: "Settlement list, count, total/settled/pending/refunded, and trend.",
      risk: { level: "low" },
    },
    // ── Lease ───────────────────────────────────────────
    {
    ...
    },
    {
      ...
      summary:
        "Submit proof through the generic proof façade while preserving service proof compatibility.",
      kind: "gateway",
      group: "market",
      availability: availability(resourcesEnabled, "resources disabled"),
      paramsSchema: {
      ...
      returns: "Generic proof submission result with proofId and linked settlement snapshot.",
      risk: { level: "medium" },
    },
    {
      name: "web3.market.proof.verify",
      summary: "Verify and summarize proof state for an order or proofId.",
      kind: "gateway",
      group: "market",
      availability: availability(resourcesEnabled, "resources disabled"),
      paramsSchema: {
      ...
      returns: "Verification status plus redacted proof summary and linked order/dispute state.",
      risk: { level: "low" },
    },
    {
      name: "web3.market.acceptance.sign",
      summary: "Buyer signs acceptance for a completed delivery and releases remaining settlement.",
      kind: "gateway",
      group: "market",
      availability: availability(resourcesEnabled, "resources disabled"),
      paramsSchema: {
    ...
    },
    {
      ...
      summary: "Buyer rejects delivery/proof and opens a dispute-ready acceptance record.",
      kind: "gateway",
      group: "market",
      availability: availability(resourcesEnabled, "resources disabled"),
      paramsSchema: {
        type: "object",
        required: ["actorId", "orderId", "reason"],
        properties: {
          actorId: { type: "string", pattern: "^0x[a-fA-F0-9]{40}$" },
          orderId: { type: "string" },
          proofId: { type: "string" },
    ...
    },
    {
      ...
      summary:
        "Aggregate order, lease, proof, acceptance, settlement, dispute, and trace into one execution summary.",
      kind: "gateway",
      group: "market",
      availability: availability(resourcesEnabled, "resources disabled"),
      paramsSchema: {
