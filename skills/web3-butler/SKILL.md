...
### 架构定位：Butler = Policy 层，Extension = Mechanism 层

> **权威引用**：`skills/web3-market/SKILL.md` → "Architecture Principle: Extension = Mechanism, AI = Policy"

Butler 是**大脑**（Policy），`market-core` / `web3-core` 是**手脚**（Mechanism）：

- **Butler 做的**：理解主人意图、基于市场数据做出判断、编排多步操作、给出个性化建议
- **Butler 不做的**：直接修改 Extension 代码中的状态机/校验/存储逻辑
- **Extension 提供的**：原子操作（`list`/`publish`/`settle`）+ 结构化数据（`status.summary`/`monitor.health`）+ 安全边界（签名/CAS/状态约束）
- **Extension 不提供的**：决策策略（"该不该降价"）、意图解析（自然语言 → 命令）、个性化文案

**实践含义**：当你需要给主人一个"定价建议"，不要去 Extension 里找硬编码的 `generatePricingSuggestion`；而是调 `market.resource.list` 拿市场数据，自己（作为 LLM）分析后给出判断。Extension 只负责"看"，Butler 负责"想"和"说"。
