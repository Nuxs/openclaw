...
> **版本**：v1.4 (Discovery & PayFi Collaborative)
> **目标**：以 **Agentic Commerce** 为核心，分阶段实现 KYA、PayFi 流支付、x402 自动支付及去中心化资源发现。[x] 在这个文档里表示已完成
...
## 📅 阶段四：发现网络 (Discovery Network - New Strategic Phase)

**时间**：Week 9+ (并行/协同开发)
**核心目标**：引入去中心化 P2P 网络栈，打破“局域网”限制，实现真正的开放市场资源发现。与盟友 Agent 协同推进。

### 4.1 P0: 协议选型与基础连接 (Week 9)

- [ ] **调研与选型**：对比 libp2p (JS/Go via sidecar) vs 自研轻量级 Gossip，确定最适合 Agent 运行环境的方案。
- [ ] **节点标识 (PeerID)**：复用或扩展现有 DID/Key 体系，生成稳定的网络节点标识。
- [ ] **Bootstrap 节点**：部署首批种子节点，实现节点握手与连接建立。

### 4.2 P1: 资源广播与索引 (Week 10)

- [ ] **Gossip 协议实现**：实现 `web3.discovery.gossip`，替代现有的 HTTP `web3.index.gossip`，支持 topic 订阅与消息泛洪。
- [ ] **分布式索引 (DHT)**：(可选) 实现 Kademlia DHT 或类似机制，支持 `find_provider` 操作。
- [ ] **目录同步**：确保本地资源目录能通过 P2P 网络同步给邻居节点。

### 4.3 P2: 路由与连通性 (Week 11)

- [ ] **NAT 穿透**：集成 STUN/TURN 或利用 Relay 节点，解决家庭/办公网络下的连接问题。
- [ ] **端到端加密通道**：在 P2P 连接之上建立加密隧道，传输 `web3.market` 协议数据（Offer/Lease）。
- [ ] **验收**：无需中心化配置，新节点启动后能自动发现网络并获取资源列表。

---

## 📅 阶段五：Week 12-13 生产可用性 (GA Readiness)

**时间**：Week 12-13  
**核心目标**：把“功能可用”收敛为“生产可运营”，完成可靠性、合规与发布闭环。

### 5.1 可靠性硬化（Reliability）

- [ ] **重试预算统一**：`WalletPolicy.autoPay.maxRetries`、`web3.billing.handlePaymentRequired`、Gateway 重试决策三处口径一致。
- [x] **故障降级自动化**：当 `x402.autopay.failure.rate` 超阈值，自动降级为“手动支付/只读调用”（已在 Gateway 健康守卫落地）。
- [ ] **幂等冲突治理**：对 `x-idempotency-key` 冲突建立分级响应（拒绝、审计、告警）。

### 5.2 运营可观测（Observability）

- [ ] **统一仪表盘**：固化 `x402.autopay.*` 与 `settlement.*` 指标大盘（成功率、失败率、重试、熔断、超额拦截）。
- [ ] **值班 Runbook**：形成“告警 -> 判因 -> 降级 -> 恢复”的标准处置流程（30 分钟内闭环）。
- [x] **审计可追溯**：关键链路（发票、支付、重试、结算）具备 request-id/idempotency-key 贯穿检索能力（新增 `web3.billing.paymentTrace.query` 与 trace 回传字段）。

### 5.3 合规与发布门禁（Compliance & Release）

- [ ] **最小权限审查**：确认策略默认拒绝高风险动作，白名单显式放行。
- [ ] **发布包门禁**：发布说明、回滚脚本、演练记录三件套齐备。
- [ ] **验收**：完成一次“灰度上线 -> 告警触发 -> 自动降级 -> 恢复放量”的全链路演练。

---

## 🧭 30/60/90 天执行视图（Execution View）

### D+30（稳定闭环）

- 完成 Phase 5.1/5.2 的核心条目。
- `x402.autopay.failure.rate` 与 `circuit_breaker.trips` 告警规则进入值班轮值。
- 交付物：稳定性报告 + 演练记录（至少 2 次）。

### D+60（规模化灰度）

- `metered` 在白名单租户中达到可持续运行（连续 2 周无 P0）。
- 动态策略灰度覆盖主要高价值工具链路。
- 交付物：租户分层策略、放量报告、失败案例复盘。

### D+90（生态化能力）

- DID/KYA 绑定完成最小可用版，支持跨主体身份校验。
- Agent 信用画像可用于市场撮合权重（可解释、可审计）。
- 交付物：对外能力说明 + 内部审计报告 + 下一季度 Roadmap。
...
- [ ] 指标命名、阈值、告警动作统一到单一配置源。
- [ ] 增加“零样本不告警”与“高失败自动降级”验证用例。

### Week C: 灰度放量

- [ ] 从 5% -> 20% -> 全量分批推进，每批都保留回滚窗口。
- [ ] 每批形成一页复盘：指标、异常、处置、结论。

### Week D: GA 评审

- [ ] 召开发布评审会（研发/测试/运维/安全/产品）。
- [ ] 形成 GA 决策：放量、延期或降级运行。

---

## 🔬 测试矩阵（Phase 1/1.5/2/3/5）
...
- [x] 告警触发后的自动降级策略已验证（Gateway 健康守卫 + 回归测试通过）。
- [ ] 值班 Runbook 可在 30 分钟内完成处置闭环。
- [x] 观测链路支持 request-id/idempotency-key 全链追踪（payment trace 落库 + 检索已完成）。
- [ ] 灰度放量（5%/20%/100%）每阶段均有复盘结论。
