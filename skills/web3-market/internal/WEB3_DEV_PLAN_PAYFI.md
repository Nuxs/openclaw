...
- [ ] 将 `WalletPolicy.autoPay.maxRetries` 接入网关重试策略（与 `x-idempotency-key` 联动，缺失幂等键默认拒绝自动支付）。
...
- [ ] **Invoice 过期**：预期返回 `PAYMENT_EXPIRED`，不发生扣款。
- [ ] **链上超时**：预期触发重试上限并记录审计，不无限重试。
- [ ] **支付成功但回调失败**：预期可幂等恢复，且账本与结算一致。
- [ ] **幂等键缺失/冲突**：预期拒绝自动支付或返回 `IDEMPOTENCY_CONFLICT`。
...
- [ ] 三份文档（架构/计划/进度）术语一致（以 `WEB3_DEV_PLAN_PAYFI.md` 为主口径）。
- [ ] 关键日志与指标可观测（至少覆盖 `x402.autopay.*` 与 `settlement.*` 关键指标）。
- [ ] 回滚演练（关闭 `web3.x402.autopay.enabled`、降级 `metered -> one-shot`）通过。
- [ ] 发布说明包含风险与回滚步骤。

### 门禁证据索引（发布评审必填）

- [ ] **术语一致性映射表**：架构/计划/进度文档中的术语与状态映射结果。
- [ ] **观测证据包**：仪表盘链接、告警规则、最近一次演练日志。
