...
> **版本**：v1.4 (Execution Ready + Discovery Sync)
> **目标**：以 **Agentic Commerce** 为核心，分阶段实现 KYA、PayFi 流支付、x402 自动支付，并与盟友文档对齐 Discovery P0/P1（P2 由盟友主文档维护）。[x] 在这个文档里表示已完成
...
**时间**：Week 9+
**核心目标**：在不改动盟友 P2 主方案的前提下，补齐我方 P0/P1 规划与集成接口，确保 PayFi 与 Discovery 可并行落地。

### 4.1 P0：协议选型与基础连接（对齐盟友）
...
- [x] **P2（NAT 穿透/Relay/端到端路由）**：由盟友文档主导维护与排期。
- [x] **我方责任**：只维护 P0/P1 的接入约束、契约一致性与回退机制；不重复规划 P2 细节，按盟友基线整合。
