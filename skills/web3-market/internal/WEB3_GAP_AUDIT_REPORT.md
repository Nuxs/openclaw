...
## 十、里程碑口径对齐（与 `WEB3_DEV_PLAN_PAYFI.md` v1.4）

> 仅做术语与编号对齐；不新增技术承诺，不覆盖盟友文档。

| 口径项       | 统一结论                                                               |
| ------------ | ---------------------------------------------------------------------- |
| Phase 4 范围 | 本计划仅维护 Discovery **P0/P1**（我方接入与集成面）                   |
| P2 归属      | Discovery **P2**（NAT/Relay/端到端路由）由盟友主文档维护与排期         |
| 我方职责边界 | 维护 `web3.index.*` / `market.resource.*` 的对接、契约一致性与回退策略 |
| 风险兜底     | P2P 异常时回退静态/HTTP 索引，不阻塞 `web3.market.*` 可用性            |
| 验收口径     | 采用“双路径验收”：P2P 可用 + 回退可用，并输出一致性/时延对账           |
