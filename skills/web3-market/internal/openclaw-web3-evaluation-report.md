...
- **双栈路线（TON+EVM，统一口径）**：已补齐"支付双入口、结算单出口、最小披露与零泄露"的规划口径（见 `docs/web3/WEB3_DUAL_STACK_STRATEGY.md` 与 `docs/reference/web3-dual-stack-payments-and-settlement.md`）；EVM 与 TON 双栈均已实现，`market-core` 的 EVM escrow 已通过 `@openclaw/blockchain-adapter` 执行层发送合约交易；TON 侧结算能力已纳入主线，详见 `docs/web3/TON_E2E_SETTLEMENT.md`。

**关键判断**：

- **架构边界与安全默认做得正确**：`market.*` 作为内核接口，默认受访问控制；工具输出默认脱敏，避免把 Token、Endpoint、真实路径泄露到外部消息通道。
- **可用性仍是最大短板**：资源发现/接入仍偏"运维式"，需要外部配置或带外分发 Provider endpoint；但 UI 已有 Market 看板（以状态与运维提示为主），离"面向普通用户的一体化市场"仍有距离。
...
| 质量 | 评分系统 / 信誉分 / 反作弊     | ⚠️   | 已有最小 `reputation` 汇总与信号，但闭环仍需加强                          |
| 经济 | 代币发行/通胀/销毁/治理        | ⚠️   | 已有 `tokenEconomy` 最小状态机与接口，链上闭环待推进                      |
| 双栈 | TON+EVM 支付/回执/结算口径统一 | ⚠️   | 统一口径与主路径已实现；仍有边界能力待补（如 TON provider 事件轮询 TODO） |
| 跨链 | 多链资产与跨链桥               | ⚠️   | 已有 `bridge` 最小状态机与接口，真实桥接执行待推进                        |
| UI   | 面向用户的市场 Dashboard       | ⚠️   | 已有 Market 看板与状态汇总；更完整的市场交互待推进                        |
...
- **双栈口径持续收敛（TON+EVM）**：按 `docs/web3/WEB3_DUAL_STACK_STRATEGY.md` 持续对齐"支付回执 + 对账摘要"统一输出；TON 主路径已可用，后续补齐边界能力（如 provider 事件轮询）。
