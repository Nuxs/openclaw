...
### 2.3 当前对齐状态与剩余边界（避免口径漂移）

- **agent-wallet 插件**：主仓已存在 `extensions/agent-wallet`，`web3-core` 已提供 `web3.wallet.*` 聚合入口；`agent-wallet.*` 保留为低层插件命名空间。
- **结算广播路径**：`market-core` 的 escrow（EVM）已通过 `@openclaw/blockchain-adapter` 执行层广播合约交易；现阶段重点是把该事实持续写清，并保证 `settlement.mode=contract` 旧配置行为不变。
- **能力入口收敛**：/web3 面板与 UI 继续只依赖 `web3.*` 稳定入口，避免直接调用 `agent-wallet.*`（防止多入口扩散）。
- **当前边界**：`web3.wallet.*` 已落地，但是否可用、稳定性与风险提示应以 `web3.capabilities.*` 当前输出为准；TON `sign` 仍未支持，需明确写出。
