...
- **位置**：
  - `extensions/web3-core/src/capabilities/catalog.ts`（`web3.resources.lease` returns/示例）
...
- **验收**：`web3.capabilities.describe("web3.resources.lease")` 输出与实际返回体一致。

### 1.2 对外输出点的“脱敏一致性”缺少单点验收
...
### 2.1 `web3.capabilities.describe` 的 `paramsSchema` 与文档/实现不一致
...
### 2.2 `web3.billing.summary` 的 `paramsSchema` 漏掉 `sessionIdHash?`

- **影响/位置/动作/验收**：同上（补齐 catalog 里的参数提示）。

### 2.3 已注册的编排 tools 未被 catalog 覆盖
...
- **Agent Wallet**：本轮只做文档归位与依赖清单，不推进实现（见 `web3-agent-wallet-plan.md`）。
