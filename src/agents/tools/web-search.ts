...
import {
  ...
  isPrivateWebSearchProvider,
  resolvePrivateWebSearchProvider,
} from "./web-search/private-hooks.js";
import {
  CacheEntry,
...
function resolveWebSearchDescription(provider: (typeof SEARCH_PROVIDERS)[number]): string {
  const privateDescription = describePrivateWebSearchProvider(provider);
  if (privateDescription) {
    return privateDescription;
  }
  if (provider === "perplexity") {
    return "Search the web using the Perplexity Search API. Returns structured results (title, URL, snippet) for fast research. Supports domain, region, language, and freshness filtering.";
  }
  if (provider === "grok") {
    return "Search the web using xAI Grok. Returns AI-synthesized answers with citations from real-time web search.";
  }
  if (provider === "kimi") {
    return "Search the web using Kimi by Moonshot. Returns AI-synthesized answers with citations from native $web_search.";
  }
  if (provider === "gemini") {
    return "Search the web using Gemini with Google Search grounding. Returns AI-synthesized answers with citations from Google Search.";
  }
  return "Search the web using Brave Search API. Supports region-specific and localized search via country and language parameters. Returns titles, URLs, and snippets for fast research.";
}
...
export function createWebSearchTool(options?: {
  ...
  const description = resolveWebSearchDescription(provider);
  return {
    label: "Web Search",
    name: "web_search",
    description,
    ...
    execute: async (_toolCallId, args) => {
      ...
      const isPrivateProvider = isPrivateWebSearchProvider(provider);
      if (!isPrivateProvider && !apiKey) {
      ...
      if (isPrivateProvider) {
        const result = await executePrivateWebSearch({
          provider,
          searchConfig: search,
          query,
          count: resolvedCount,
          searchLang: resolvedSearchLang,
          timeoutSeconds,
          cacheTtlMs,
          cache: SEARCH_CACHE,
        });
