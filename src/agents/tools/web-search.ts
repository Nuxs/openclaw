...
import { resolveConfiguredPrivateWebSearchProvider } from "../../plugins/web-search-providers.private.js";
...
function resolveProviderSecretPath(providerId: string): string {
  return providerId === "brave"
...
function hasProviderCredential(
  ...
  providers: readonly PluginWebSearchProviderEntry[] = resolvePluginWebSearchProviders({
  ...
  const fromConfig = normalizeSecretInput(
    normalizeResolvedSecretInputString({
      value: rawValue,
      path: resolveProviderSecretPath(providerId),
    }),
  );
...
function resolveSearchProvider(
  ...
  const privateProvider = resolveConfiguredPrivateWebSearchProvider({
    rawProvider: raw,
    searchConfig: search,
  });
  if (privateProvider) {
  ...
  if (!raw) {
    for (const provider of providers) {
      if (!hasProviderCredential(provider.id, search, providers)) {
        continue;
      }
      logVerbose(
...
export function createWebSearchTool(options?: {
  ...
  const providerId =
    ...
    resolveSearchProvider(search, providers);
  const provider =
    providers.find((entry) => entry.id === providerId) ??
    providers.find((entry) => entry.id === resolveSearchProvider(search, providers)) ??
    providers[0];
  if (!provider) {
    return null;
  }
