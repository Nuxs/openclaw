...
export type ParsedIpAddress = Ipv4Address | Ipv6Address;
...
const RFC2544_BENCHMARK_PREFIX: [Ipv4Address, number] = [
  ipaddr.IPv4.parse("198.18.0.0") as unknown as Ipv4Address,
  15,
];
...
function parseIpv6WithEmbeddedIpv4(raw: string): Ipv6Address | undefined {
  ...
  return ipaddr.IPv6.parse(normalizedIpv6) as unknown as Ipv6Address;
}

export function isIpv4Address(address: ParsedIpAddress): address is Ipv4Address {
  return address.kind() === "ipv4";
}

export function isIpv6Address(address: ParsedIpAddress): address is Ipv6Address {
...
export function parseCanonicalIpAddress(raw: string | undefined): ParsedIpAddress | undefined {
  ...
  if (ipaddr.IPv4.isValid(normalized)) {
    ...
    return ipaddr.IPv4.parse(normalized) as unknown as Ipv4Address;
  }
  if (ipaddr.IPv6.isValid(normalized)) {
    return ipaddr.IPv6.parse(normalized) as unknown as Ipv6Address;
  }
...
export function parseLooseIpAddress(raw: string | undefined): ParsedIpAddress | undefined {
  ...
  if (ipaddr.isValid(normalized)) {
    return ipaddr.parse(normalized) as unknown as ParsedIpAddress;
  }
  return parseIpv6WithEmbeddedIpv4(normalized);
}
...
function decodeIpv4FromHextets(high: number, low: number): Ipv4Address {
  ...
  return ipaddr.IPv4.parse(octets.join(".")) as unknown as Ipv4Address;
}

export function extractEmbeddedIpv4FromIpv6(address: Ipv6Address): Ipv4Address | undefined {
  if (address.isIPv4MappedAddress()) {
    return address.toIPv4Address();
  }
...
export function isIpInCidr(ip: string, cidr: string): boolean {
  ...
  try {
    parsedCidr = ipaddr.parseCIDR(candidate) as unknown as [ParsedIpAddress, number];
  } catch {
    return false;
  }
