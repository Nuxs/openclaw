...
export async function loadUsage(
  ...
  try {
    ...
    try {
      const billingRes = await client.request("web3.billing.summary", {
        sessionKey: state.sessionKey,
      });
      state.usageBillingSummary = normalizeBillingSummary(billingRes);
      state.usageBillingError = null;
    } catch (err) {
      state.usageBillingSummary = null;
      state.usageBillingError = toErrorMessage(err);
    }
  } catch (err) {
